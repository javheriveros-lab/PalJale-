...
async def nearby_products(lat: float = Query(...), lng: float = Query(...), max_km: float = Query(20.0), category: Optional[str] = None, transaction_type: Optional[str] = None, q: Optional[str] = None, limit: int = Query(50, le=100), user: UserContext = Depends(get_current_user)):
    db = await get_db()
    pipeline = []
